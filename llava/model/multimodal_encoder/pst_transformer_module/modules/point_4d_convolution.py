...
class P4DConv(nn.Module):
    def __init__(
        ...
        for i in range(1, len(mlp_planes)):
            if mlp_planes[i] != 0:
                mlp.append(
                    nn.Conv2d(
                        in_channels=mlp_planes[i - 1],
                        out_channels=mlp_planes[i],
                        kernel_size=1,
                        stride=1,
                        padding=0,
                        bias=bias,
    ...
    def forward(
        self, xyzs: torch.Tensor, features: torch.Tensor = None
        ...
        assert (
            self.temporal_kernel_size % 2 == 1
        ), "P4DConv: Temporal kernel size should be odd!"
        assert (
            nframes + sum(self.temporal_padding) - self.temporal_kernel_size
        ...
        if self.in_planes != 0:
            ...
            features = [
                torch.squeeze(input=feature, dim=1).contiguous() for feature in features
            ]
        ...
        for t in range(
            ...
            len(xyzs) - self.temporal_kernel_size // 2,
            self.temporal_stride,
        ):  # temporal anchor frames
            # spatial anchor point subsampling by FPS
            anchor_idx = pointnet2_utils.furthest_point_sample(
                xyzs[t].float(), npoints // self.spatial_stride
            )  # (B, N//self.spatial_stride)
            anchor_xyz_flipped = pointnet2_utils.gather_operation(
                xyzs[t].float().transpose(1, 2).contiguous(), anchor_idx
            )  # (B, 3, N//self.spatial_stride)
            anchor_xyz_expanded = torch.unsqueeze(
                anchor_xyz_flipped, 3
            )  # (B, 3, N//spatial_stride, 1)
            anchor_xyz = anchor_xyz_flipped.transpose(
                1, 2
            ).contiguous()  # (B, N//spatial_stride, 3)

            new_feature = []
            for i in range(
                t - self.temporal_kernel_size // 2,
                t + self.temporal_kernel_size // 2 + 1,
            ):
                neighbor_xyz = xyzs[i]
                ...
                neighbor_xyz_flipped = neighbor_xyz.transpose(
                    1, 2
                ).contiguous()  # (B, 3, N)
                neighbor_xyz_grouped = pointnet2_utils.grouping_operation(
                    neighbor_xyz_flipped, idx
                )  # (B, 3, N//spatial_stride, k)

                xyz_displacement = (
                    neighbor_xyz_grouped - anchor_xyz_expanded
                )  # (B, 3, N//spatial_stride, k)
                t_displacement = torch.ones(
                    (
                        xyz_displacement.size()[0],
                        1,
                        xyz_displacement.size()[2],
                        xyz_displacement.size()[3],
                    ),
                    dtype=torch.float32,
                    device=device,
                ) * (i - t)
                ...
                if self.spatial_pooling == "max":
                    feature = torch.max(input=feature, dim=-1, keepdim=False)[
                        0
                    ]  # (B, out_planes, n)
...
class P4DTransConv(nn.Module):
    def __init__(
        ...
        for i in range(len(mlp_planes)):
            if i == 0:
                conv.append(
                    nn.Conv1d(
                        in_channels=in_planes + original_planes,
                        out_channels=mlp_planes[i],
                        kernel_size=1,
                        stride=1,
                        padding=0,
    ...
    def forward(
        ...
        original_xyzs = [
            torch.squeeze(input=original_xyz, dim=1).contiguous()
