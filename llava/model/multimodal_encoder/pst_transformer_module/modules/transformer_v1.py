...
class Attention(nn.Module):
    ...
    def forward(self, xyzs, features):
        ...
        q, k, v = map(
            lambda t: rearrange(t, "b l n (h d) -> b h (l n) d", h=h), qkv
        )  # [b, h, m, d]

        xyzs_flatten = rearrange(xyzs, "b l n d -> b (l n) d")  # [b, m, 3]

        delta_xyzs = torch.unsqueeze(input=xyzs_flatten, dim=1) - torch.unsqueeze(
            input=xyzs_flatten, dim=2
        )  # [b, m, m, 3]
        ...
        delta_xyzs = torch.sum(
            input=attn * delta_xyzs, dim=3, keepdim=False
        )  # [b, h, m, 3]
