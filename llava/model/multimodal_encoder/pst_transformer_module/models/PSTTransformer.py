...
class PSTTransformerLegacy(nn.Module):
    def __init__(
        ...
        self.transformer = Transformer(
            dim, depth, heads, dim_head, mlp_dim, dropout=dropout1
        )
...
class PSTTransformer(nn.Module):
    ...
    def forward(self, batch):  # [B, L, N, 3]
        ...
        output = self.transformer(xyzs, features)
        output = torch.max(input=output, dim=1, keepdim=False, out=None)[
            0
        ]  # for the sake of llava training we get multiple tokens
