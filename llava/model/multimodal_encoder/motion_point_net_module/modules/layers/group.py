...
class QueryAndGroup(nn.Module):
    ...
    def forward(
        ...
        extended_xyz = (
            query_xyz.transpose(-2, -1).unsqueeze(-1).repeat(1, 1, 1, self.nsample)
        )
        dist = self.pdist(extended_xyz, grouped_xyz)
        gaussion_density = torch.exp(-dist / (2.0 * self.radius * self.radius)) / (
            2.5 * self.radius
        )
        ...
        grouped_xyz = torch.cat(
            (grouped_xyz, gaussion_density.unsqueeze(-3)), dim=-3
        )  # dim = 3+1
...
def get_aggregation_feautres(p, dp, f, fj, feature_type="dp_fj"):
    ...
    if feature_type == "dp_fj":
        ...
        fj = torch.cat(
            [p.transpose(1, 2).unsqueeze(-1).expand(-1, -1, -1, dp.shape[-1]), dp, fj],
            1,
        )
...
if __name__ == "__main__":
    ...
    # --------------- debug downsampling
    from openpoints.models.layers.layer3d import (
        RandomSample,
        random_sample,
