...
from src.models.pipelines.cl4d_module.cl4d_pipeline import CL4D


@dataclass
class CL4DConfig:
    """
    Configuration class for CL4D motion encoder.
    """

    model_name: str = "cl4d-motion-encoder"
    ...
    # CL4D specific configs (will be loaded from checkpoint)
...
class CL4DVisionTower(nn.Module):
    """
    Vision tower wrapper for CL4D motion encoder.
    ...
    def __init__(self, vision_tower, args, delay_load=False):
        """
        Args:
            vision_tower: str, path to checkpoint directory (e.g., "output/DynAction4DHuman/CL4D")
    ...
    def load_model(self, device_map=None):
        """
        Load the CL4D motion encoder from checkpoint.
        """
        if self.is_loaded:
            print(
        ...
        print(f"Loading CL4D motion encoder from: {self.checkpoint_dir}")
        ...
        print("Creating CL4D pipeline...")
        self.vision_tower = CL4D(feature_mode="all", **pipeline_args)
    ...
    def feature_select(self, motion_features):
        ...
        For CL4D with feature_mode='all', the output is already in the format we need:
        (batch, num_tokens, hidden_size)
        """
        # CL4D with feature_mode='all' returns: [batch, num_tokens, hidden_size]
    ...
    def forward(self, motion_data):
        """
        Forward pass through the CL4D motion encoder.
    ...
    def num_patches_per_side(self):
        ...
        For CL4D, this is derived from the ViT architecture.
        """
    ...
    def num_patches(self):
        ...
        For CL4D with feature_mode='all', this depends on the ViT configuration.
...
def build_cl4d_vision_tower(vision_tower, args, **kwargs):
    """
    Factory function to build the CL4D vision tower.
    This is called by the builder.
    """
    return CL4DVisionTower(vision_tower, args=args, **kwargs)
